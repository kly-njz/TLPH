...
@role_required('municipal','municipal_admin')
def applicants_municipal_view(job_id):
    db = get_firestore_db()
    user_municipality = (_resolve_municipality_from_user_context() or '').strip()
    user_region = (_resolve_region_from_user_context() or '').strip()
    ...
    try:
        ...
        if not doc.exists:
            return render_template(
                'municipal/operations/applicant-detail-municipal.html',
        ...
        if scope_type and scope_type != 'municipality':
            return render_template(
                'municipal/operations/applicant-detail-municipal.html',
                error='Access denied: this record is not municipality-scoped.',
                applicant={}
            ), 403
        ...
        if scope_key and scope_key != normalize_scope(user_municipality):
            return render_template(
                'municipal/operations/applicant-detail-municipal.html',
                error='Access denied for this municipality scope.',
                applicant={}
            ), 403
        ...
        if normalize_scope(data.get('municipality_key') or data.get('municipality')) != normalize_scope(user_municipality):
            return render_template(
                'municipal/operations/applicant-detail-municipal.html',
                error='Access denied for this municipality.',
                applicant={}
            ), 403

        if normalize_scope(data.get('region_key') or data.get('region') or data.get('region_office')) != normalize_scope(user_region):
            return render_template(
                'municipal/operations/applicant-detail-municipal.html',
        ...
        applicant = {
            ...
            'scope_type': scope_type or 'municipality',
            'scope': data.get('scope') or data.get('municipality') or 'N/A',
        ...
        print(f"[ERROR] Failed to render municipal applicant detail: {e}")
        return render_template(
            'municipal/operations/applicant-detail-municipal.html',
            applicant={},
...
def api_municipal_project_mark_completed(project_id):
    ...
    try:
        ...
        update_payload = {
            ...
            'regional_completed': project.get('regional_completed', False),
            'updated_at': firestore.SERVER_TIMESTAMP,
