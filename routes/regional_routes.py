...
@role_required('regional', 'regional_admin')
def api_get_regional_coa_templates():
    """Get all COA templates across municipalities in the region"""
    try:
        # Get region from session
        region_session = session.get('region', '')
        if not region_session:
            return jsonify([]), 200
        
        db = get_firestore_db()
        
        # Get municipalities in region
        municipalities_ref = db.collection('municipalities').document(region_session)
        municipalities_doc = municipalities_ref.get()
        
        if not municipalities_doc.exists:
            return jsonify([]), 200
        
        municipalities_list = municipalities_doc.get('municipalities', [])
        
        # Fetch all templates from all municipalities in the region
        all_templates = []
        for municipality in municipalities_list:
            try:
                query_result = db.collection('coa_templates').where('municipality', '==', municipality).stream()
        ...
        print(f"[ERROR] Getting regional COA templates: {e}")
        return jsonify([]), 200
...
@role_required('regional', 'regional_admin')
def api_get_regional_coa_accounts():
    """Get COA accounts across municipalities in the region"""
    try:
        # Get region from session
        region_session = session.get('region', '')
        if not region_session:
        ...
        if template_id:
            ...
            for municipality in municipalities_list:
                try:
                    # First get templates for this municipality
                    templates = db.collection('coa_templates').where('municipality', '==', municipality).stream()
                    for template_doc in templates:
                        template_id_iter = template_doc.id
                        # Then get accounts for each template
                        accounts = db.collection('coa_accounts').where('template_id', '==', template_id_iter).stream()
        ...
        print(f"[ERROR] Getting regional COA accounts: {e}")
        return jsonify([]), 200